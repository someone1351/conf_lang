...
impl<'a> RecordContainer<'a> {
    ...
    pub fn walk(&self,mut callback : impl for<'b> FnMut(Walk<'b>) -> Option<RecordContainer<'a>>) -> Result<(),WalkError<()>> { //'a,
        if self.conf.is_none() {return Ok(());};
        traverse(*self, |w|Ok(callback(w)))
    }

    pub fn walk_ext<E:Debug>(&self,callback : impl for<'b> FnMut(Walk<'b>) -> Result<Option<RecordContainer<'a>>,(E,Option<Loc>)>) -> Result<(),WalkError<E>> { //'a,
        if self.conf.is_none() {return Ok(());};
